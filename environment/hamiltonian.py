...
class HamiltonianConfig(enum.Enum):
    ...
    # Future entries (e.g. NEXT_NEAREST_NEIGHBORS, FULLY_CONNECTED) go here.
    # build_hamiltonian() will raise NotImplementedError for unimplemented ones.


def make_jx(d: int) -> torch.Tensor:
...
def build_hamiltonian(
    ...
    ----------
    jx:
        J_x spin operator of shape (d, d), dtype cfloat.
        Precomputed once via :func:`make_jx`.
    phis:
    ...
    """
    if h_config is not HamiltonianConfig.NEAREST_NEIGHBORS:
        raise NotImplementedError(
            f"HamiltonianConfig.{h_config.name} is not yet implemented. "
            "Only NEAREST_NEIGHBORS is currently supported."
        )
...
def displacement_pulse(
    ...
    Parameters
    ----------
    jx:    J_x spin operator of shape (d, d)
    ...
    """
    H = build_hamiltonian(jx, phis, h_config)
