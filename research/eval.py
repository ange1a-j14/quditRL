...
Loads a checkpoint written by train.py, rebuilds the matching policy, and
runs greedy episodes on held-out target unitaries. Reports final fidelity,
diagnostic L1 distance, and pulse count for each target.
...
def main():
    ...
    # Eval only reads info metrics, so no reward shaping
